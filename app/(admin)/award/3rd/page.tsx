...
// 3位表彰画面（進行側）
export default function Award3rdPage() {
    ...
    const fetchParticipants = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (ranked.length >= 3) {
            setParticipant(ranked[2]); // 3位（0-indexedなので2）
          }
        }
      } catch (error) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-8">
      <div className="w-full max-w-2xl space-y-8 rounded-lg bg-white p-8 shadow-lg">
        <div className="text-center">
          <h1 className="mb-4 text-5xl font-bold text-yellow-600">3位</h1>
