...
// 2位表彰画面（進行側）
export default function Award2ndPage() {
    ...
    const fetchParticipants = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (ranked.length >= 2) {
            setParticipant(ranked[1]); // 2位（0-indexedなので1）
          }
        }
      } catch (error) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-8">
      <div className="w-full max-w-2xl space-y-8 rounded-lg bg-white p-8 shadow-lg">
        <div className="text-center">
          <h1 className="mb-4 text-5xl font-bold text-gray-400">2位</h1>
